...
namespace ThinkInvisible.ClassicItems {
    public class Embryo : Item<Embryo> {
        ...
        public override void SetupLate() {
            ...
            foreach(var k in hooksEnabled.Keys)
                BindRoO(FindConfig(nameof(hooksEnabled), k), new AutoConfigRoOCheckboxAttribute());
        ...
        private int _CheckEmbryoProc(CharacterBody body) {
            ...
            return Mathf.FloorToInt(totalChance / 100f) + (Util.CheckRoll(totalChance % 100f, body?.master) ? 1 : 0);
        }
        ...
        private int _CheckEmbryoProc(Inventory inv) {
            ...
            return Mathf.FloorToInt(totalChance / 100f) + (Util.CheckRoll(totalChance % 100f, inv?.gameObject?.GetComponent<CharacterMaster>()) ? 1 : 0);
        }
